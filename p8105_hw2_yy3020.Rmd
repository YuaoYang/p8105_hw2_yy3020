...
the number of observations in trash_data is (`r nrow(trash_data) `) and in new_pp is (`r nrow(new_pp)`). The key variables are shown below. The total precipitation in 2018 is (`r sum(pull(pp_2018,total)) `)。the median number of sports balls in a dumpster in 2017 is (`r filter(trash_data, year == 2017) %>% pull(sports_balls) %>% median `). 
